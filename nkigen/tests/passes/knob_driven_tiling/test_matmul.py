...
def test_matmul_tiling(M, N, K, tile_size, reduction_tile, request):
    ...
    Memref backend generates:
      for block_m in [0, M, BLOCK_M):
        transpose LHS block into SBUF
        for block_n in [0, N, BLOCK_N):
          promote RHS block to SBUF
          for tile_m in [0, BLOCK_M, TILE_M):
            for tile_n in [0, BLOCK_N, TILE_N):
              alloc PSUM accumulator
              for k in [0, K, TILE_K):
                matmul_transpose_a
    """
    ...
    CHECK: memref.alloc() : memref<{K}x{block_m}xf32, #nkipy.mem<Sbuf>>
    CHECK: linalg.transpose {{{{.*}}}} permutation = [1, 0]
    CHECK: scf.for %{{{{.*}}}} = %c0{{{{.*}}}} to %c{N}{{{{.*}}}} step %c{block_n}
    CHECK: memref.alloc() : memref<{K}x{block_n}xf32, #nkipy.mem<Sbuf>>
    CHECK: scf.for %{{{{.*}}}} = %c0{{{{.*}}}} to %c{block_m}{{{{.*}}}} step %c{tile_m}
    CHECK: scf.for %{{{{.*}}}} = %c0{{{{.*}}}} to %c{block_n}{{{{.*}}}} step %c{tile_n}
    CHECK: memref.alloc() : memref<{tile_m}x{tile_n}xf32, #nkipy.mem<Psum>>
